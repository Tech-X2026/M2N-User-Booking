...
const login = async (req, res) => {
    try {
        ...
        const receptionist = await prisma_1.default.receptionist.findUnique({ where: { email } });
        if (receptionist) {
            const isMatch = await bcryptjs_1.default.compare(password, receptionist.password);
            if (isMatch) {
                res.json({
                    id: receptionist.id,
