...
const GlobalCategoryManager: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/global-categories/${editingId}`, { name, amenities }, {
          headers: { Authorization: `Bearer ${user?.token}` }
        });
      } else {
        const response = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/global-categories`, { name, amenities }, {
          headers: { Authorization: `Bearer ${user?.token}` }
        });
  ...
  const handleArchive = async (id: string) => {
    if (window.confirm('Are you sure you want to archive this category?')) {
      try {
        await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/global-categories/${id}`, {
          headers: { Authorization: `Bearer ${user?.token}` }
        });
        fetchCategories();
      } catch (error) {
