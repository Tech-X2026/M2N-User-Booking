...
const ArchivedCategoryManager: React.FC = () => {
  ...
  const fetchArchivedCategories = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/global-categories?archived=true`, {
  ...
  const fetchArchivedHotels = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/hotels?archived=true`, {
        headers: { Authorization: `Bearer ${user?.token}` }
      });
      setHotels(data);
    } catch (error) {
  ...
  const handleRestoreCategory = async (id: string) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/global-categories/${id}/restore`, {}, {
        headers: { Authorization: `Bearer ${user?.token}` }
      });
      fetchArchivedCategories();
    } catch (error) {
  ...
  const handleRestoreHotel = async (hotelId: string) => {
    ...
    if (window.confirm(msg)) {
      try {
        const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/hotels/${hotelId}/restore`, {}, {
          headers: { Authorization: `Bearer ${user?.token}` }
        });
